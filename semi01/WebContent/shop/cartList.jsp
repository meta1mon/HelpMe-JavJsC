...
<body>
	...
	%>
		...
		<img src="../imageFile/<%=cartList.getBimage() %>" border="0" width="30" height="50" align="middle">
		<%=cartList.getBtitle() %>
		</td>
		<td width="100"><%=NumberFormat.getInstance().format(cartList.getBuyPrice()) %></td>
		<td width="150" align="center">
		<input type="text" name="buyCount" size="5" value="<%=cartList.getBuyCount() %>">
		<%
		...
		<input type="button" value="삭제" onclick="javascript:window.location='cartListDel?list=<%=cartList.getCid() %>
		&bkind=<%=bkind %>'">
		</td>
		</tr>
		<%} %>
		...
		<td width="300" align="left">
		<img src="../imageFile/<%=cartList.getVimage() %>" border="0" width="30" height="50" align="middle">
		<%=cartList.getVtitle() %>
		</td>
		<td width="100"><%=NumberFormat.getInstance().format(cartList.getvBuyPrice()) %></td>
		<td width="150" align="center">
		<input type="text" name="buyCount" size="5" value="1">
		<%
		String url = "updateCartForm?cid="+cartList.getCid()+"&bkind="+bkind+"&buyCount="+cartList.getBuyCount();
		%>
		<input type="button" value="수정" onclick="javascript:window.location='<%=url %>>'">
		...
		<b>총 금액 :<%=NumberFormat.getInstance().format(total+vtotal) %></b></td>
		</tr>
		<tr>
		<td colspan="5">
		<input type="button" value="구매하기" onclick="javascript:window.location='butForm'">
		<input type="button" value="쇼핑 계속하기" onclick="javascript:window.location='introList?bkind=<%=bkind %>'">
		<input type="button" value="장바구니 비우기" onclick="javascript:window.location='cartListDel?list=all&bkind=<%=bkind %>'">
		</td>
		</tr>
		</table>
		</form>
