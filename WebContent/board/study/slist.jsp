...
<body class="content">
	<div style="width: 840px; margin: 0 auto 0 auto; color: #99ADB6; background:#ffffff; padding:20px; border-radius: 4px;">
		...
		<input id="write" type="button" value="새 글 쓰기"
			onclick="location.href = 'board/study/swrite.jsp'">
		...
		<table id="table">
			<c:forEach items="${slist }" var='s'>
				<tr>
					...
					<td style="width: 50px; font-size: 14px;"><a
						style="font-size: 18px;">${s.rstudycnt }</a><br>댓글</td>
		...
		<div id="page">
			<c:if test="${startPage !=1 }">
				<a href="studylist?pageNum=${startPage -1 }&search=${search }&searchType=${searchType}" id="leftpage">&#60;&#60;</a>
			</c:if>

			<c:forEach begin="${startPage }" end="${endPage }" var="p" step="1">
				<a href="studylist?pageNum=${s }&search=${search }&searchType=${searchType}" id="pageNum">&nbsp;${p }&nbsp;</a>
			</c:forEach>

			<c:if test="${endPage < pageBoxCnt }">
				<a href="studylist?pageNum=${endPage +1 }&search=${search }&searchType=${searchType}" id="rightpage">&#62;&#62;</a>
			</c:if>
		</div>
	</div>
</body>
