...
<body class="content">
	<div style="width: 840px; margin: 0 auto 0 auto; color: #99ADB6; background:#ffffff; padding:20px; border-radius: 4px;">
		<div class="review">Review 게시판</div><br>
		<table id="table">
			<tr>
				<td>
					<div id="writer">${review.rwriter }</div> <br>
					<div id="date">${review.rdate}</div>
					<div id="views">
					...
					<div id="answer">
						<img src="<%=request.getContextPath()%>/images/speechbubble.png"
							id="speechbubble"> ${review.rreviewcnt }&nbsp;
					</div> <br>
					<hr>
				</td>
			</tr>
			<tr>
				<td>
					<div id="tag">
						<c:choose>
							<c:when test="${review.rtag ==1}">Java</c:when>
							<c:when test="${review.rtag ==2}">C</c:when>
							<c:when test="${review.rtag ==3}">Python</c:when>
						</c:choose>
					</div> <br>
					<div id="subject">${review.rsubject }</div>

					<hr style="position: relative; top: 7px;">
					<div id="qlikeid">
						<img src="<%=request.getContextPath()%>/images/like.png"
							onclick="rlike()" style="cursor: pointer;"> <br>
						${review.rlikecnt }
					</div>
					<div id="content">
						${review.rcontent }<br>
						<div id="file">
							...
							<c:forTokens var="fileName" items="${review.rfilepath}" delims=","
								varStatus="st">
								<a download="${fileName}"
									href="<%=request.getContextPath() %>/board/review/files/${fileName }">${fileName}</a>
								<c:if test="${!st.last }">
                        /
                    </c:if>
								<br>
		...
		<c:if test="${reply != null}">
			<c:forEach items="${reply }" var="r">
					<div id="rlike">
						<img src="<%=request.getContextPath() %>/images/like.png" onclick="rrlike(${r.rrno})" style="cursor: pointer;"> <br>
						${r.rrlikecnt }
					</div>
					<div id="rcontent">${r.rrwriter}<br>${r.rrcontent }
				...
				<form action="<%=request.getContextPath()%>/rreviewwrite" method="post">
					<div style="float: right; margin-top: 10px;">
						<input type="hidden" name="rno" value="${review.rno }">
						<textarea placeholder="댓글 쓰기" id="editor" name="rrcontent"
			...
			<button type="button" id="list"
				onclick="location.href='<%=request.getContextPath()%>/reviewlist'">목록으로
				돌아가기</button>
	...
	function rlike() {
		...
		$.ajax({
			url : "<%=request.getContextPath()%>/rlike",
			type : "post",
			data : {
				id : "${loginMember.id}",
				rno : "${review.rno}"
...
// rqlike는 몇 번째 댓글인지 알아와야 되는데, 어떻게 할지 모르겠어서 그냥 하트 이펙트 뺐음...
	function rrlike(rrno) {
		if(${loginMember == null}) {
			alert("로그인이 필요합니다");			
		} else {
			$.ajax({
				url : "<%=request.getContextPath()%>/rrlike",
				type : "post",
				data : {
					id : "${loginMember.id}",
					rrno : rrno
